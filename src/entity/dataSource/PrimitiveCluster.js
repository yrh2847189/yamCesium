...
class PrimitiveCluster {
     ...
     */
    getLabel(collectionProperty = "_labelCollection", CollectionConstructor = Cesium.LabelCollection, unusedIndicesProperty = "_unusedLabelIndices", entityIndexProperty = "labelIndex") {
        return (entity) => {
     ...
     * @param entityIndexProperty
     */
    getBillboard(collectionProperty = "_billboardCollection", CollectionConstructor = Cesium.BillboardCollection, unusedIndicesProperty = "_unusedBillboardIndices", entityIndexProperty = "billboardIndex") {
        return (entity) => {
            let collection = collectionProperty;
            if (!Cesium.defined(this._collectionIndicesByEntity)) {
                this._collectionIndicesByEntity = {};
     ...
     * @param entityIndexProperty
     */
    getPoint(collectionProperty = "_pointCollection", CollectionConstructor = Cesium.PointPrimitiveCollection, unusedIndicesProperty = "_unusedPointIndices", entityIndexProperty = "pointIndex") {
        return (entity) => {
            let collection = collectionProperty;
            if (!Cesium.defined(this._collectionIndicesByEntity)) {
                this._collectionIndicesByEntity = {};
...
function getBoundingBox(item, coord, pixelRange, primitiveCluster, result) {
    ...
    if (primitiveCluster._clusterLabels &&
        ...
        const labelIndex = primitiveCluster._collectionIndicesByEntity[item.id.id].labelIndex;
...
function createDeclutterCallback(primitiveCluster) {
    return function (amount) {
        ...
        let clusteredBillboardCollection = primitiveCluster._clusterBillboardCollection;
        ...
        if (currentHeight < previousHeight) {
            ...
            for (i = 0; i < length; ++i) {
                ...
                // @ts-ignore
                const coord = Cesium.Billboard._computeScreenSpacePosition(Cesium.Matrix4.IDENTITY, cluster.position, Cesium.Cartesian3.ZERO, Cesium.Cartesian2.ZERO, scene);
                if (!Cesium.defined(coord)) {
                    continue;
                }
        ...
        for (i = 0; i < length; ++i) {
            ...
            const totalBBox = Cesium.BoundingRectangle.clone(bbox, totalBoundingRectangleScratch);
            ...
            for (j = 0; j < neighborLength; ++j) {
                ...
                if (!neighborPoint.clustered) {
                    ...
                    const neighborBBox = getBoundingBox(neighborItem, neighborPoint.coord, pixelRange, primitiveCluster, neighborBoundingRectangleScratch);
            ...
            if (numPoints >= minimumClusterSize) {
                const position = Cesium.Cartesian3.multiplyByScalar(clusterPosition, 1.0 / numPoints, clusterPosition);
