...
export default class PlotStraightArrowDrawer {
  ...
  showModifyStraightArrow(options: any) {
    let _this = this;
    var arr = [];
    ...
    return new Promise((resolve, reject) => {
      if (options && options.confirmHandler) {
        ...
        options.confirmHandler().then(() => {
          const p1 = _this.positions[0];
          const p2 = _this.positions[1];
          const firstPoint = _this._getLonLat(p1);
          const endPoints = _this._getLonLat(p2);
  ...
  startDrawStraightArrow(options: any) {
    let _this = this;
