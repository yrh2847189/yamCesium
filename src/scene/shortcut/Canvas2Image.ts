...
export const Canvas2Image = (function() {
  ...
  function saveFile(strData: string, fileType: string, fileName = "screenShot") {
  ...
  function fixType(type: string) {
    ...
    const r = type.match(/png|jpeg|bmp|gif/);
    if (r && r.length > 0) {
      return "image/" + r[0];
    }
    return "";
   ...
   * 按照规则生成图片响应头和响应体
   */
  const genBitmapImage = function(oData: ImageData) {
    //
   ...
   * @param width {Number} [optional] png width
   * @param height {Number} [optional] png height
   * @param type {string} [optional] image type
   * @param fileName {String} image name
   */
  const saveAsImage = function(canvas: HTMLCanvasElement | string, width: number, height: number, type: string, fileName: string) {
    // save file type
    const fileType = type;
    if ($support.canvas && $support.dataURL) {
      ...
      type = fixType(type);
      if (/bmp/.test(type)) {
        const data = getImageData(scaleCanvas(canvas, width, height)) as ImageData;
  ...
  const convertToImage = function(canvas: HTMLCanvasElement | string, width: number, height: number, type: string) {
    if ($support.canvas && $support.dataURL) {
      if (typeof canvas == "string") {
        canvas = document.getElementById(canvas) as HTMLCanvasElement;
      }
  ...
  return {
    ...
    convertToPNG: function(canvas: HTMLCanvasElement, width: number, height: number) {
      return convertToImage(canvas, width, height, "png");
    },
    convertToJPEG: function(canvas: HTMLCanvasElement, width: number, height: number) {
      return convertToImage(canvas, width, height, "jpeg");
    },
    convertToGIF: function(canvas: HTMLCanvasElement, width: number, height: number) {
      return convertToImage(canvas, width, height, "gif");
    },
    convertToBMP: function(canvas: HTMLCanvasElement, width: number, height: number) {
