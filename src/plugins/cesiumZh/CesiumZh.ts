 ...
 * 4、针对cesium 1.58版本汉化
 */
export const CesiumZh = (function() {
  /**
   * 调用汉化方法
   * @param {*} viewer
   */
  ...
  function loadBaseLayerPickerOfToolbar() {
    //图层选择需要汉化对象
    let layers: any = getChineseMaps();

    //获取容器
    let dropDownContainer = document.querySelectorAll(".cesium-baseLayerPicker-dropDown") as NodeListOf<HTMLDivElement>;
    if (dropDownContainer.length > 0) {
      let container = dropDownContainer[0];
      //修改标题
      updateHtml(container, layers, "cesium-baseLayerPicker-sectionTitle");
      //副标题
      updateHtml(container, layers, "cesium-baseLayerPicker-categoryTitle");
      //具体标签
      updateHtml(container, layers, "cesium-baseLayerPicker-itemLabel");
      //具体地图描述
      updateAttribute(container, layers, "cesium-baseLayerPicker-item", "title");

      //鼠标悬浮事件监听，解决选择不同影像title 动态修改无法固定汉化问题
      let selectLayerToolbar = document.querySelectorAll(".cesium-baseLayerPicker-selected") as NodeListOf<HTMLDivElement>;
      if (selectLayerToolbar.length > 0) {
        let toolbar = selectLayerToolbar[0];
        if (!toolbar || !toolbar.parentElement) {
          return;
        }
        toolbar.parentElement.onmouseover = function(event: MouseEvent) {
          let target = event.currentTarget as HTMLElement;
          let currentTitle = target.getAttribute("title");
          ...
          target.setAttribute("title", tempTitle1 + "\n" + tempTitle2);
        };
      }
    }
  }
  ...
  function loadToolBarHelp() {
    let texts: any = getChineseMaps();

    //鼠标
    let mouseButtons = document.querySelectorAll(".cesium-navigation-button-left") as NodeListOf<HTMLButtonElement>;
    let mouseButton: HTMLButtonElement, textContent: string;
    if (mouseButtons && mouseButtons.length > 0) {
      mouseButton = mouseButtons[0];
      textContent = mouseButton.textContent ? mouseButton.textContent : "";
      if (texts[textContent])
        mouseButton.innerHTML = mouseButton.children[0].outerHTML + texts[textContent];
    }

    let touchButtons = document.querySelectorAll(".cesium-navigation-button-right") as NodeListOf<HTMLButtonElement>;
    let touchButton: HTMLButtonElement;
    if (touchButtons && touchButtons.length > 0) {
      touchButton = touchButtons[0];
      textContent = touchButton.textContent ? touchButton.textContent : "";
      if (texts[textContent])
        touchButton.innerHTML = touchButton.children[0].outerHTML + texts[textContent];
    }
  ...
  function loadFlyController() {
    let titles: any = getChineseMaps();
    let parentContainers = document.querySelectorAll(".cesium-viewer-animationContainer") as NodeListOf<HTMLButtonElement>;
    let parentContainer: HTMLElement;
    if (parentContainers && parentContainers.length > 0) {
      parentContainer = parentContainers[0];
      ...
      for (let i = 0; i < datedes.length; i++) {
        ...
        if (texts.length == 3) {
          ...
          datedes[i].addEventListener("DOMNodeInserted", function(e) {
            ...
            target.nodeValue = target.nodeValue.split(" ")[0] + "标准时间";
          });
        }
      }
    }
    ...
    if (cesiumDescription.length > 0) {
      let insertEvent = function(e: any) {
        e = e || event;
    ...
    let timeLineBars = document.querySelectorAll(".cesium-timeline-bar") as NodeListOf<HTMLButtonElement>;
    let timeLineBar: HTMLElement;
    if (timeLineBars && timeLineBars.length > 0) {
      timeLineBar = timeLineBars[0];
      timeLineBar.addEventListener("DOMNodeInserted", function(e) {
        e = e || event;
        let target: any = e.target;
        if (target.nodeType == 3)
          return;
        if (target.getAttribute("class") == "cesium-timeline-ticLabel") {
          let texts = target.innerHTML.split(" ");
          if (texts.length > 3) {
            let resultText = texts[2] + "年" + titles[texts[0]] + texts[1] + "日 " + texts[3] + "";
   ...
   */
  function updateHtml(parent: any, map: any, className: string) {
    if (!parent)
      parent = document;
   ...
   * @param {*} tagName 标签名称
   */
  function updateHtmlByTagName(parent: any, map: any, tagName: string) {
    if (!parent)
      parent = document;
   ...
   */
  function updateAttribute(parent: any, map: any, className: string, attributeName: string) {
    if (!parent)
      parent = document;
   ...
   */
  function updateAttributeByEvent(parent: any, map: any, className: string, attributeName: string, eventName: string) {
    let doms = parent.getElementsByClassName(className);
    if (doms.length > 0) {
      for (let i = 0; i < doms.length; i++) {
        doms[i][eventName] = function(event: any) {
